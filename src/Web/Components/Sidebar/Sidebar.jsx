...
const Sidebar = () => {
  ...
  // Mock user ranking data for sidebar
  const userRankData = {
    globalRank: 128,
    tier: 'Silver',
    progressToNext: 75
  ...
  // 🔹 Page Headings
  const pageConfigs = {
    "/fan/dashboard": { heading: "My Account" },
    "/fan": { heading: "Invite" },
    "/fan": { heading: "Campaigns & Promotions" },
    "/fan": { heading: "Rankings" },
    "/fan": { heading: "My X-KRYPTED Items" },
    "/fan": { heading: "My Tickets" },
    "/fan": { heading: "Wallet Transactions" },
    "/fan": { heading: "Store" },
    "/fan": { heading: "Transfer F̈" },
  };
          ...
          <motion.div
            ...
            style={{
              background: isScrolled 
                ? '#333759' 
                : 'linear-gradient(to_bottom, #19417F 0%, #364172 100%)'
            }}
          >
                    ...
                    <div className="flex flex-col space-y-2">
                      {section.items.map((link, linkIndex) => {
                        ...
                        return (
                          ...
                          >
                            <motion.div
                              className={`w-[95%] py-3 ms-2 ps-2 rounded-sm transition-all duration-300 ${isHighlighted
                                ? "bg-white text-black"
                                ...
                                {link.showRank && (
                                  <div className="flex flex-col items-end">
                                    <div className="flex items-center gap-2">
                                      <span className={`text-sm font-bold ${isHighlighted ? 'text-black' : getTierColor(userRankData.tier)}`}>
                                        #{userRankData.globalRank}
                                      </span>
                                      <span className={`text-xs ${isHighlighted ? 'text-gray-600' : 'text-gray-300'}`}>
                                        {userRankData.tier}
                                      </span>
                                    </div>
                                    <div className="w-16 h-1 bg-gray-600 rounded-full mt-1">
                                      <div
                                        className="h-1 bg-gradient-to-r from-[#f64c68] to-[#ff6b6b] rounded-full transition-all duration-500"
                                        style={{ width: `${userRankData.progressToNext}%` }}
                                      ></div>
                                    </div>
                                  </div>
                                )}
