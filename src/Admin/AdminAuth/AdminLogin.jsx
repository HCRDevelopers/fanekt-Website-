...
function AdminLogin() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await login(credentials); // ✅ Call your API
  ...
  return (
    <div
      className="min-h-screen w-full flex justify-center items-center 
                 bg-[linear-gradient(to_bottom_right,_#2d2e58,_#2d2e58)] lg:h-[100vh]"
    >
        <Toaster />
      {/* Left Side Image */}
      ...
      {/* Right Side Form */}
      <div className="w-full lg:w-[50%] flex flex-col items-center justify-center px-3 md:px-6 bg-[radial-gradient(circle,_#4a3052_0%,_#2d2e58_100%)] h-full">
        <motion.img
          animate={{ y: [0, -20, 0] }}
          transition={{
            duration: 2,
            ...
            <div className="w-full flex justify-center">
              <button
                ...
                className="w-full max-w-[400px] py-3 text-white text-lg text-center font-medium rounded-lg cursor-pointer
                           ...
                           hover:opacity-90 transition-all duration-300 shadow-md disabled:opacity-70"
              >
