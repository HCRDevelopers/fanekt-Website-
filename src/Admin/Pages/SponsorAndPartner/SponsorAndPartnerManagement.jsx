...
import SponsorAndPartnerDetailModal from "./Modals/SponsorAndPartnerDetailModal";
import { getAllSponsorUsers } from "../../../API/apiService";

const SponsorAndPartnerManagement = () => {
  ...
  // Fetch sponsor users data
  const fetchSponsorUsers = useCallback(async () => {
    setLoading(true);
    try {
      ...
      if (data?.status && data?.data?.users) {
        ...
        const transformedData = data.data.users.map(user => ({
          id: user.id,
          companyFullLegalName: user.sponsor?.full_name || user.name,
          brandAKA: user.sponsor?.brand_name || user.name,
          appearInSearch: user.sponsor?.aka_name || user.name,
          legalRepresentativeName: user.sponsor?.legal_represent || null,
          companyLoginEmail: user.email,
          legalRepEmail: user.sponsor?.sec_email || null,
        ...
        setFilteredSponsors(transformedData);
  ...
  useEffect(() => {
    ...
    const filtered = sponsors.filter(
      (sponsor) =>
        sponsor.companyFullLegalName?.toLowerCase().includes(lowerSearch) ||
        sponsor.companyLoginEmail?.toLowerCase().includes(lowerSearch) ||
        sponsor.brandAKA?.toLowerCase().includes(lowerSearch) ||
        sponsor.city?.toLowerCase().includes(lowerSearch)
    );
  ...
  return (
    <div className="lg:ml-[290px] px-4 lg:px-6 py-3 lg:pb-5 lg:pt-5 pt-28">
      ...
      <div className="flex flex-wrap gap-4 justify-between">
        <div>
          <p className="text-2xl font-medium text-white">Sponsor and Partner Management</p>
          <p className="text-[14px] font-medium text-[#ffffff8b]">
            Monitor and manage all registered sponsors and partners
          </p>
        </div>
      </div>
                    ...
                    filteredSponsors.map((sponsor, index) => (
                      ...
                      >
                        <td className="px-4 lg:px-6 py-6 text-white/90 border-r border-white/20 text-center whitespace-nowrap">
                          {sponsor.companyFullLegalName}
                        </td>
                        <td className="px-4 lg:px-6 py-6 text-white/90 border-r border-white/20 text-center whitespace-nowrap">
                          {sponsor.companyLoginEmail}
                        </td>
                        <td className="px-4 lg:px-6 py-6 text-white/90 border-r border-white/20 text-center whitespace-nowrap">
                          {sponsor.brandAKA}
                        </td>
...
export default SponsorAndPartnerManagement;
