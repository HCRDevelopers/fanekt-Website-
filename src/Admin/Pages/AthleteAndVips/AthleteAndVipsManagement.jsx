...
import AthleteAndVipsDetailModal from "./Modals/AthleteAndVipsDetailModal";
import { getAllAthleteUsers } from "../../../API/apiService";

const AthleteAndVipsManagement = () => {
  ...
  // Fetch athlete users data
  const fetchAthleteUsers = useCallback(async () => {
    setLoading(true);
    try {
      const response = await getAllAthleteUsers();
      ...
      if (data?.status && data?.data?.users) {
        ...
        const transformedData = data.data.users.map(user => ({
          id: user.id,
          fullLegalName: user.athlete?.full_name || user.name,
          nickname: user.athlete?.nick_name || null,
          appearInSearch: user.athlete?.how_appear || user.name,
          sport: user.athlete?.sport || null,
          email: user.email,
      ...
      console.error("Error fetching athlete users:", error);
      setAthletes([]);
      setFilteredAthletes([]);
      setTotal(0);
      setLastPage(1);
  ...
  useEffect(() => {
    ...
    const filtered = athletes.filter(
      (athlete) =>
        athlete.fullLegalName?.toLowerCase().includes(lowerSearch) ||
        athlete.email?.toLowerCase().includes(lowerSearch) ||
        athlete.sport?.toLowerCase().includes(lowerSearch) ||
        athlete.city?.toLowerCase().includes(lowerSearch)
    );
  ...
  return (
    <div className="lg:ml-[290px] px-4 lg:px-6 py-3 lg:pb-5 lg:pt-5 pt-28">
      ...
      <div className="flex flex-wrap gap-4 justify-between">
        <div>
          <p className="text-2xl font-medium text-white">Athlete and VIPs Management</p>
          <p className="text-[14px] font-medium text-[#ffffff8b]">
            Monitor and manage all registered athletes and VIPs
          </p>
        </div>
      </div>
                  ...
                  {loading ? (
                    ...
                    filteredAthletes.map((athlete, index) => (
                      ...
                      >
                        ...
                        <td className="px-4 lg:px-6 py-6 text-center">
                          <button onClick={() => openAthleteModal(athlete)}>
                            <img
                              src={View}
...
export default AthleteAndVipsManagement;
